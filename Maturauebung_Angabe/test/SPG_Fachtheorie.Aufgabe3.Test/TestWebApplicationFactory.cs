...
public class TestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            var descriptor = services.First(d => d.ServiceType == typeof(DbContextOptions<ExamsContext>));
            services.Remove(descriptor);
            services.AddDbContext<ExamsContext>(options =>
            {
                options.UseSqlite("DataSource=exams_test.db");
            });
        });
        builder.UseEnvironment("Testing");
    }
    public void InitializeDatabase(Action<ExamsContext> action)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<ExamsContext>();
        db.Database.EnsureDeleted();
        db.Database.EnsureCreated();
        action(db);
    }

    public Tout QueryDatabase<Tout>(Func<ExamsContext, Tout> query)
    {
        using var scope = Services.CreateScope();
        using var db = scope.ServiceProvider.GetRequiredService<ExamsContext>();
        return query(db);
